...
const PageAddListing2: FC<PageAddListing2Props> = () => {
  ...
  const onPlaceChanged = () => {
    if (autocomplete) {
      ...
      if (place.geometry) {
        const { lat, lng } = place?.geometry?.location?.toJSON() as any;
    ...
    <form onSubmit={handleSubmit(onHandleFormSubmit)} className="listingSection__wrap">
      ...
      <div className="space-y-8">
         ...
         <FormItem label="Pool Address">
          <Input placeholder="Enter Pool Address" {...register('pool_address', { required: 'Pool address is required' })} />
          {errors.pool_address && <span  className="text-red-600 text-sm">{errors.pool_address.message}</span>}
        </FormItem>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-5">
          <FormItem label="Pool Width">
            <Input  {...register('pool_width', { required: 'Pool width is required' })} />
            {errors.pool_width && <span  className="text-red-600 text-sm">{errors.pool_width.message}</span>}
          </FormItem>
          <FormItem label="Pool Length">
            <Input {...register('pool_length', { required: 'Pool length is required' })} />
            {errors.pool_length && <span  className="text-red-600 text-sm">{errors.pool_length.message}</span>}
          </FormItem>
          <FormItem label="Pool Depth">
            <Input  {...register('pool_depth', { required: 'Pool depth is required' })} />
            {errors.pool_depth && <span  className="text-red-600 text-sm">{errors.pool_depth.message}</span>}
          </FormItem>
        </div>
