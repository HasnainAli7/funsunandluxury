...
const stripePromise = loadStripe("pk_test_51N9968DXoT0g7l2jTYN7H1Carf6I2M3fpmIUREZbcYEjxmiWFIwLXQ60TH67qYa5uu8vaxDe5KyY64KY1eJetKpA00JYUBghzO");
...
const ListingStayDetailPage: FC<ListingStayDetailPageProps> = ({ params }: any) => {
  ...
  const fetchPoolDetails = async () => {
    ...
    try {
      const [responseDetails, responseListings] = await Promise.all([
        fetch('/api/pool/pool-detail/' + params.Id),
        fetch('/api/pool_booking_listing/' + params.Id)
      ]);
      ...
      const blockOutDatesArray = detailsData.PoolDetails[0].blockout_dates
        .split(",")
      ...
      setPHOTOS(detailsData.PoolDetails[0].pool_images.split(","));
      settotalamountValue(Number(detailsData.PoolDetails[0].guest_rate_1_5));
      setpoolListings(detailsData.PoolDetails);
  ...
  useEffect(() => {
    calculateRate(totalGuests, hoursInputValue);
  }, [totalGuests, hoursInputValue]);
  ...
  const calculateRate = (guests: number, hours: number) => {
    ...
    const rates = [
      { min: 1, max: 5, rate: poolListings[0].guest_rate_1_5 },
      { min: 6, max: 10, rate: poolListings[0].guest_rate_6_10 },
      { min: 11, max: 20, rate: poolListings[0].guest_rate_11_20 },
      { min: 21, max: 30, rate: poolListings[0].guest_rate_21_30 },
      { min: 31, max: 40, rate: poolListings[0].guest_rate_31_40 },
      { min: 41, max: 50, rate: poolListings[0].guest_rate_41_50 },
      { min: 51, max: 65, rate: poolListings[0].guest_rate_51_65 },
      { min: 66, max: 80, rate: poolListings[0].guest_rate_66_80 },
      { min: 81, max: 100, rate: poolListings[0].guest_rate_81_100 }
    ];

    let hourlyRate = 0;
    for (const rateObj of rates) {
      if (guests >= rateObj.min && guests <= rateObj.max) {
        hourlyRate = Number(rateObj.rate);
        break;
      }
    }

    const finalRate = hourlyRate * hours;
    settotalamountValue(finalRate);
  };
  ...
  async function createBooking() {
    ...
    const booking = {
      ...
      ListingType: 'Pool',
      StoreListingID: poolListings[0].id,
    ...
    try {
      ...
      const response = await fetch('/api/pool_booking_listing', {
        method: 'POST',
        headers: {
    ...
    try {
      const res = await fetch('/api/create-checkout-session', {
        ...
        body: JSON.stringify({
          ...
          currency: 'usd',
          description: poolListings && poolListings[0] ? poolListings[0].pool_title : "",
          bookingId: Number(Id),
          Booking_Listing_type: "POOL LISTING",
          CreatedBy:user?.id
        }),
      });
    ...
    return (
      <div className="listingSection__wrap !space-y-6">
        ...
        <div className="flex justify-between items-center">
          <Badge name="POOL LISTING" />
          {/* <LikeSaveBtns /> */}
        </div>

        {/* 2 */}
        <h2 className="text-2xl sm:text-3xl lg:text-4xl font-semibold">
          {poolListings && poolListings[0] ? poolListings[0].pool_title : ""}
        </h2>

        {/* 3 */}
        <div className="flex items-center space-x-4">
          {poolListings && poolListings[0] &&
            <StartRating point={Number(Number(poolListings[0].average_rating).toFixed(2))} reviewCount={poolListings[0].review_count} />
          }
          <span>·</span>
          <span>
            <i className="las la-map-marker-alt"></i>
            <span className="ml-1"> {poolListings && poolListings[0] ? poolListings[0].city + " " + poolListings[0].state : ""}</span>
          </span>
        </div>
    ...
    return (
      <div className="listingSection__wrap">
        ...
        <div className="text-neutral-6000 dark:text-neutral-300">
          <span>
            {poolListings && poolListings[0] ? poolListings[0].description : ""}
          </span>
  ...
  const renderSection6 = () => {
    return (
      <div className="listingSection__wrap">
        ...
        {
          poolListings && poolListings[0] &&
          <h2 className="text-2xl font-semibold">Reviews ({poolListings[0].review_count} reviews)</h2>
        }
        <div className="w-14 border-b border-neutral-200 dark:border-neutral-700"></div>
        ...
        <div>
          ...
          <span className="block mt-2 text-neutral-500 dark:text-neutral-400">
            {poolListings.length > 0 && (
              poolListings[0].Country + " " + poolListings[0].city
            )}
          </span>
        </div>
          ...
          <div className="rounded-xl overflow-hidden z-0">
            {poolListings.length > 0 && (
              <iframe
                ...
                src={`https://www.google.com/maps/embed/v1/place?key=AIzaSyDz7rPf5wTgxQt0ZxMl4gNuxtO5Rd5H1Sk&q=${poolListings[0].latitude},${poolListings[0].longitude}`}
              ></iframe>
            )}
          </div>
        </div>
      ...
      <div className="listingSectionSidebar__wrap shadow-xl">
        ...
        <div className="flex justify-between">
          <span className="text-3xl font-semibold">
            ${Number(poolListings && poolListings[0] ? poolListings[0].guest_rate_1_5 : "0")}
  ...
  return (
    <div className="nc-ListingStayDetailPage">
     ...
     {
      loading && <div className="flex justify-center mt-5 items-center" style={{height:"100%",position:"absolute",top: 0,left: 0,right: 0,bottom: 0,zIndex: 1}}>
          <div role="status">
        ...
        <div className="relative grid grid-cols-3 sm:grid-cols-4 gap-1 sm:gap-2">
          ...
          >
            <Image
              ...
              src={poolListings && poolListings[0] ? poolListings[0].pool_cover_image : ""}
              alt=""
